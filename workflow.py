...
# utils
from globals import UNIT_OPS, UNIT_PX_SCALARS, PALETTE_OPS, PROG_COLOR_1, PROG_COLOR_2, REAL_COLOR, RAND_COLOR
...
class WorkflowPage(QWidget):
    ...
    def __init__(self, wf: WorkflowObj, coords: List[Tuple[float, float]], alt_coords: List[Tuple[float, float]] = None,
        ...
        self.gen_rand_cb.clicked.connect(
            partial(self.create_visuals, self.wf, (self.bars_ip.text() if self.bars_ip.text() else 'fd'),
                    self.output_ops))
        # stats (single file)
        self.gen_stats_lb = QLabel("show image analysis")
        self.gen_stats_lb.setStyleSheet("margin-left: 50px; font-size: 17px; font-weight: 400;")
        self.gen_stats_cb = QCheckBox()
        self.gen_stats_cb.clicked.connect(
            partial(self.create_visuals, self.wf, (self.bars_ip.text() if self.bars_ip.text() else 'fd'),
                    self.output_ops))
        # cb row
        cb_row = QHBoxLayout()
        ...
        cb_row.addWidget(self.gen_stats_lb)
        cb_row.addWidget(self.gen_stats_cb)
        ...
        self.wf_props = [self.run_btn, self.image_frame, self.graph_frame, self.gen_rand_cb, self.gen_real_cb, self.gen_stats_cb]
        # run on init
    ...
    def create_visuals(self, wf: WorkflowObj, n_bins, output_ops: OutputOptions, n: List[int] = np.zeros(11)):
        ...
        try: 
            matplotlib.use('agg')
            if self.gen_real_cb.isChecked() or self.gen_rand_cb.isChecked() or self.gen_stats_cb.isChecked() and len(self.coords) > 0:
                ...
                if wf["graph"]["type"] == "hist":
                    ...
                    if self.gen_stats_cb.isChecked():
                        ax.set_title(f'{wf["graph"]["title"]} (Further Analysis)')
                        cm = sns.color_palette(self.r_pal_type.currentText(), as_cmap=True)
                        graph_df = self.data.final_real[wf["graph"]["x_type"]]
                        #ax.scatter(x=graph_df, y=)
                        #ax = sns.lineplot(graph_df, err_style = 'bars') # https://www.youtube.com/watch?v=xm6zDqyRigY
                        # ax = sns.barplot(data=graph_df, errwidth= 2.5)
                        sns.swarmplot(graph_df)
                        # plt.xticks(size = 10, weight = "bold")
                        # plt.yticks(size = 10, weight = "bold")
                        #ax = plt.axes(projection="3d") # https://www.youtube.com/watch?v=fAztJg9oi7s
                    #    plt.autoscale() 


# Conceptual component of project (theoretical, trial and error implementations of multi-image analysis)
       
                    # if globals.multi_folder_btn_func.isChecked():
                    #     if self.gen_stats_cb.isChecked(): 
                    #         ax.set_title(f'{wf["graph"]["title"]} (Random)')
                    #         cm = sns.color_palette(self.r_pal_type.currentText(), as_cmap=True)
                    #         graph_df = self.data.final_rand[wf["graph"]["x_type"]]

                    #     #image = HomePage.multi_folders_func()
                    #     image_list = []
                    #     for image in image_list:
                    #         ax.hist(image, bins=(int(n_bins) if n_bins.isdecimal() else n_bins),
                    #         alpha=0.75, color=create_color_pal(n_bins=1, palette_type=self.pal_type.currentText()),
                    #         label='Real')
                    #         ax.set_title(f'{wf["graph"]["title"]} (Multi-File Analysis)')
                        # multi_image_graphs = self.multi_folders[wf["graph"]["x_type"]] # self.data.final_real[wf["graph"]["x_type"]]
                        # ax.hist(multi_image_graphs, bins=(int(n_bins) if n_bins.isdecimal() else n_bins),
                        #                            alpha=0.75, color=create_color_pal(n_bins=1, palette_type=self.pal_type.currentText()),
                        #                            label='Real')

        #               for image in self.multi_folders:
        #                   image = self.data.final_real_df1[wf["graph"]["x_type"]]
        #                   ax.hist(image, bins=(int(n_bins) if n_bins.isdecimal() else n_bins), alpha=0.75,
        #                       color=create_color_pal(n_bins=1, palette_type=self.r_pal_type.currentText()), label='Test1')
        #                   ax.set_title(f'{wf["graph"]["title"]} Multi-File Analysis')
        #                   ax.legend(loc='upper right')    

                    #if self.gen_stats_cb.isChecked() and HomePage.open_multi_folder_picker == True: 
                    #    cm = sns.color_palette(self.r_pal_type.currentText(), as_cmap=True)
                    #    for image in len(self.multi_folders['image']): 
                    #        self.data.final_stats = pixels_conversion(
                    #                data=self.data.real_df1, unit=Unit.PIXEL, scalar=float(output_ops.output_scalar))
                    #        image = self.data.final_stats 
                            #ax.boxplot(image)
                            #ax.show()
                        #for image in len(self.multi_folders): # looping over images to plot simultaneously 
                        #    ax.figure() 
                        #    ax.boxplot(image)
                        #    ax.show()
                        #ax.boxplot(graph_df) # plot two boxplots on top of each other 
                        # graph_df = HomePage.multi_folders[0]['image'][0][wf["graph"]["x_type"]]
                        # plt.show()
                        ...
                        #        graph_df = self.data.final_real[wf["graph"]["x_type"]]
                                #plt.autoscale() 
                        #        ax.boxplot(graph_df)
                                #ax.boxplot(data=graph_df, x=graph_df[0].size())
                ...
                """ ADD NEW (ANNOTATED) VISUALIZATIONS HERE """
                if wf["type"] == Workflow.NND:
